        ...
        <div id="issue" class="d-flex flex-column h-100 text-left">
            <div id="issue-header" class="mb-0 d-flex flex-column">
                <div class="d-flex align-items-center pt-2">
                    <p class="mr-auto i smaller-text mb-0">
                        ...
                        <span class="issue-status-description">
                            ...
                            <a class="nostyle" href="/users/{{$author->id}}"> <span
                                    class="author-reference">{{$author->name}}</span></a>
                ...
                <div class="d-flex align-items-center my-2">
                    ...
                    <input type="text" class="form-control d-none" id="edit-task-label" />
                    <button type="button" class="custom-button edit-button edit-task d-none ml-auto" id="save-issue"
                        data-issue-id="{{ $issue['id'] }}">
                        <i class="far fa-save"></i>
                    </button>
                    <button type="button" class="custom-button edit-button edit-task" id="edit-issue">
                ...
                <ul class="labels smaller-text d-flex align-items-center">
                    ...
                    <li class="mr-2">
                        <h6 class="label mb-0 px-1 list-item-label bg-info p-1 text-white"
                            style="background-color:{{$tag->rgb_code}};">
                            {{$tag->name}}
                        </h6>
                    </li>
                    @endforeach
                    <li>
                        <button type="button" class="d-none custom-button add-button">
            ...
            <div class="row border-bottom my-2">
                <div class="col-md-9 description text-left">
                    ...
                    <input type="text" class="form-control d-none" id="edit-task-description" />
                </div>
                <div class="col-md-3 members-and-duedate pb-3 d-flex flex-column justify-content-end ml-auto">
                    <div class="assignees-container ml-auto">
                        <ul class="assignees d-flex align-items-center justify-content-center pb-3">

                            @foreach (\App\User::join('assigned_user', 'user.id', '=',
                            'assigned_user.user_id')->where('assigned_user.issue_id', '=', $issue->id )->get() as $user)
                            <li class="mr-2">
                                <a href="/users/{{$user['id']}}">
                                    <img src="{{ asset('assets/avatars/'.  $user['photo_path'] .'.png') }}"
            ...
            <div class="add-comment-container">
                ...
                <form class="d-flex">
                    <img class="comment-author"
                        src="{{ asset('assets/avatars/'.  Auth::User()['photo_path'] .'.png') }}"
                        alt="{{ Auth::User()['username']}} profile picture" />
                    <input class="w-100 mx-2 px-2" type="text" name="comment" id="new-comment"
                        placeholder=" Write a comment..." data-user="{{Auth::Id()}}" data-issue="{{$issue->id}}" />
                </form>
            </div>
            <div class="comments-container">
                ...
                <div class="comment d-flex border-bottom mt-4 pb-1">
                    <img class="comment-author" src="{{ asset('assets/avatars/'.  $comment['photo_path'] .'.png') }}"
                        alt="{{ $comment['username']}} profile picture" />

                    <div class="comment-detail ml-3">
                        <h6>
                            <a href="/users/{{$comment['user_id']}}"><span
                                    class="author-reference">{{ $comment['username']}}</span></a>
                            <span class="comment-timestamp ml-1">
                                {{ date_format(date_create($comment['creation_date']),'h:i a \o\n l jS F Y')}}</span>
                        </h6>
                        <p>
                            {{$comment['content']}}
                        </p>
