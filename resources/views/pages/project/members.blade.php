...
<div class="main-content-container px-4">
    ...
    <div id="tables-types" class="d-flex border-bottom nav-links pb-2">
        <li class="active">
            Developer <span id="developers-counter"
                class="tables-type-counter">{{ count(\App\MemberStatus::where('project_id', '=', $project->id)->where('role', '=', 'developer')->join('user', 'user.id', '=', 'member_status.user_id')->get()) }}</span>
        </li>
        <li>Coordinator <span id="coordinators-counter"
                class="tables-type-counter">{{ count(\App\MemberStatus::where('project_id', '=', $project->id)->where('role', '=', 'coordinator')->join('user', 'user.id', '=', 'member_status.user_id')->get()) }}</span>
        </li>
        <li>All <span id="total-counter"
                class="tables-type-counter">{{ count(\App\MemberStatus::where('project_id', '=', $project->id)->join('user', 'user.id', '=', 'member_status.user_id')->get()) }}</span>
        </li>
        <button type="button" data-toggle="modal" data-target="#addMemberModal" class="btn btn-success ml-auto ">
    ...
    <div id="filter-issues" class="border-bottom py-3 my-0">
        <form class="d-md-flex justify-content-between">
            <div class="form-group mb-0 flex-grow-1">
                <input type="text" class="form-control" name="issues-filter" id="issues-filter"
                    placeholder="Search or filter results..." />
            </div>
            <div id="filter-buttons" class="dropdown ml-1">
                <button data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" id="dropdownFiltersButton"
                    type="button" type="submit" class="custom-button primary-button dropdown-toggle">
                    Sort by<i class="fas fa-chevron-down ml-3"></i>
                </button>
                <div class="dropdown-menu" aria-labelledby="dropdownFiltersButton">
            ...
            <div class="main-box clearfix">
                <div class="table-responsive" id="developers">
                    <table class="table user-list">
                        ...
                        <tbody>
                            @foreach (\App\MemberStatus::where('project_id', '=', $project->id)->where('role', '=',
                            'developer')->join('user', 'user.id', '=', 'member_status.user_id')->get() as $member)
                            <tr class="user_{{$member->user_id}}">
                                ...
                                <td>
                                    <a href="#" class="table-link">
                                        <button class="custom-button close-button" data-toggle="modal"
                                            data-target="#remove-member-modal" data-user="{{ $member->user_id }}"
                ...
                <div class="table-responsive d-none" id="coordinators">
                    <table class="table user-list">
                        ...
                        <tbody>
                            @foreach (\App\MemberStatus::where('project_id', '=', $project->id)->where('role', '=',
                            'coordinator')->join('user', 'user.id', '=', 'member_status.user_id')->get() as $member)
                            <tr class="user_{{$member->user_id}}">
                                <td data-label="User">
                                    <img src="{{asset('assets/avatars/' . $member->photo_path . '.png')}}" alt="" />
                                    <a href="#" class="user-link">{{ $member->username }}</a>
...
<!-- Remove Member modal -->
<div class="modal fade" id="remove-member-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Remove <span id="user-to-remove"> from the
                        project?</span></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            ...
            <div class="modal-footer">
                ...
                <button id="remove-member" type="button" data-dismiss="modal" class="btn btn-primary btn-danger">
                    Remove
                </button>
            </div>
        </div>
