...
CREATE TABLE notification_kick (
    notification_id integer PRIMARY KEY REFERENCES notification  ON DELETE CASCADE
                                                     ON UPDATE CASCADE,
    project_id integer NOT NULL REFERENCES notification ON DELETE CASCADE
                                                     ON UPDATE CASCADE
);
...
CREATE TABLE color (
    id SERIAL PRIMARY KEY,
    rgb_code text CHECK (rgb_code LIKE '[0-9A-F][0-9A-F][0-9A-F][0-9A-F][0-9A-F][0-9A-F][0-9A-F][0-9A-F]') NOT NULL
);
